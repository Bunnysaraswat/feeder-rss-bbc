        ...
        this.state.queriedData.forEach(i => feeds.push(
            <div className="card myCard" key={i.title}>
                <div className="card-body">
                    <h3 className="card-title" style={{fontfamily: 'Livvic, sans-serif'}}><a href={i.link}>{i.title}</a></h3>
                    <p className="card-text" style={{fontfamily: 'Livvic, sans-serif'}}>{i.content}</p>
                </div>
            </div>
        ));
        return (
